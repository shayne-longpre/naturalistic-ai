...
class Dataset():
    ...
    def __init__(self, dataset_id: str, data: List[Conversation] = []):
        ...
        if len(data) == 0:
            warnings.warn(
                "No Data Provided to the Dataset class. Please load data using the load_data_from_file() function.")
        if len(self.data) > 0:
    ...
    def load_data_from_file(self, path_to_dataset_downloads=""):
        ...
        if path_to_use.endswith('.json'):
            ...
            raise ValueError(
                "Unsupported file format. Only .json, .jsonl, and .csv are supported.")

        self.data = conversations
        print("Data loaded! Access using .data, .get_dataloader, or .to_pandas")
        return
