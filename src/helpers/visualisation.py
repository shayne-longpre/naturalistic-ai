    ...
    if all(isinstance(v, (int, float)) for v in data.values()):
        ...
        if order == "ascending":
            ...
            sns.barplot(x='Category', y='Count',
                        hue='Source', data=all_data, ax=ax)
...
def plot_confusion_matrix(
    ...
    sns.heatmap(matrix, annot=True,
                fmt='.2f' if normalize else 'd', cmap=cmap, ax=ax)
    ax.set_xlabel(xlabel)
    ax.tick_params(axis='x', rotation=90)
    ax.set_ylabel(ylabel)
    ax.set_title(title)
...
def analyze_pair_annotations(
    ...
    for uid, val1, val2 in annotations:
        ...
        if order_matters:  # If we do care which side is which:
            ...
            normalized_pairs.append((uid, norm_val1, norm_val2)
                                    if norm_val1 < norm_val2 else (uid, norm_val2, norm_val1))
...
def run_interrater_comparison(
    ...
    outdir = os.path.join(
        outdirpath, "{annotation_source_1}--{annotation_source_2}/{task_name}")
    ...
    fig2 = plot_confusion_matrix(info_to_plot_cm, normalize=True, xlabel="", ylabel="",
                                 title="Confusion Matrix", output_path=f"{outdir}/confusion_matrix.png")
    ...
    print()
    print(f"-----------------{task_name}-----------------")
    print({k: round(v, 3) if isinstance(v, float)
          else v for k, v in agreement_metrics.items()})
